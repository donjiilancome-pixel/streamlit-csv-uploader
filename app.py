    ...
    if realized_f.empty or "実現損益[円]" not in realized_f.columns:
        ...
        if r.empty:
            ...
            for label,col in [("日別","日"),("週別","週"),("月別","月"),("年別","年")]:
                ...
                st.write(f"**{label}**")
                # st.dataframe を使用してソート可能にする
                st.dataframe(
                    disp[["日付","実現損益[円]"]],
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "実現損益[円]": st.column_config.NumberColumn(format="%,d円")
                    }
                )
    ...
    if realized_f.empty or "実現損益[円]" not in realized_f.columns:
        ...
        if d.empty:
            ...
            st.dataframe(
                ...
                column_config={
                    "実現損益[円]": st.column_config.NumberColumn(format="%,d円"),
                    "累計": st.column_config.NumberColumn(format="%,d円"),
                }
            )
    ...
    if realized_f.empty or "実現損益[円]" not in realized_f.columns:
        ...
        if not sym.empty:
            ...
            st.dataframe(
                ...
                use_container_width=True, hide_index=True,
                column_config={
                    "実現損益合計": st.column_config.NumberColumn(format="%,d円"),
                    "1回平均損益": st.column_config.NumberColumn(format="%,d円"),
                    "勝率": st.column_config.NumberColumn(format="%.1f%%"),
                }
            )
...
with tab4:
    ...
    if realized_f.empty or "実現損益[円]" not in realized_f.columns:
        ...
        if d.empty:
            ...
            # ソートは数値のまま行い、表示はcolumn_configで行う
            out_sorted = out.sort_values(sort_key, ascending=False).head(topn)
            
            order = ["銘柄コード","銘柄名","実現損益合計","取引回数","1回平均損益"]
            cols = [c for c in order if c in out_sorted.columns] + [c for c in out_sorted.columns if c not in order]
            st.dataframe(
                out_sorted[cols],
                use_container_width=True, hide_index=True,
                column_config={
            ...
            download_button_df(out_sorted[cols], "⬇ CSVダウンロード（ランキング）", "ranking.csv")
...
def _nearest_pos_by_ns(idx: pd.DatetimeIndex, t0: pd.Timestamp) -> int:
    """
    idx: tz-aware な DatetimeIndex、t0 も tz-aware（JST）を想定。
    ns の差の絶対値で argmin を取り、バージョン差異を回避。
    """
    ...
    # Pandas 互換性のため asi8 を使用（int64 ns）
    idx_ns = idx.asi8
...
def align_trades_to_ohlc(ohlc: pd.DataFrame, trades: pd.DataFrame, max_gap_min=6):
    ...
    # OHLC の index を JST の DatetimeIndex にしてソート
    odf = ohlc.copy()
            ...
            if yakujyou_all is None or yakujyou_all.empty:
                ...
                if yak_day_all.empty:
                    ...
                    st.dataframe(
                        ...
                        use_container_width=True, hide_index=True,
                        column_config={
                            "価格": st.column_config.NumberColumn(format="%,.0f"),
